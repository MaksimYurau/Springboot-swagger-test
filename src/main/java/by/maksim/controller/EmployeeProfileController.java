...
import by.maksim.entity.Employee;
import by.maksim.entity.ErrorResponse;
...
@ApiOperation(value = "/profile/v1/employee", tags = "Employee Profile Container", notes = "Employee Profile API")
@RestController
@RequestMapping(path = "/profile/v1/employee")
public class EmployeeProfileController {
    ...
    public List<Employee> fetchEmployeeByName(@ApiParam(value = "Employee Name") @PathVariable(value = "name") String name) {
        return (List<Employee>) employees.stream().filter(x -> x.getName().equalsIgnoreCase(name)).findFirst().get();
    }
